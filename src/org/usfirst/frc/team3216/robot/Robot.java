...
public class Robot extends IterativeRobot {
	...
	public void robotInit() {
		...
		try {
			arduino = new SerialPort(115200,SerialPort.Port.kUSB); // 115200 baud arduino on USB port
			arduino_att = true;
		} catch (Exception e) {
			arduino_att = false;
		}
		...
		lifttracker = NetworkTable.getTable("LiftTracker"); // network tables for each one of these
	...
	public void autonomousInit() {
		...
		matchTimer.reset(); // make sure we have this reset
		matchTimer.start();
		rear_vision = -100;
		front_vision = -100;
	...
	public void autonomousPeriodic() {
		...
		if (StateMachine.isRunning("drive_fwd_3") && // drive
			...
			StateMachine.start("aim_high");  // calls the 
		}
		...
		if (StateMachine.isRunning("drive_back_1")) { // drive backward
			...
			drive(-Settings.get("autonspeed"),-Settings.get("autonspeed")); // TODO: switch if backwards, make much slower!!!
	...
	public void teleopPeriodic() {
		...
		runintake_in = xBox.getRawAxis(2) > 0.5; // LeftBumper 
		...
		autoshoot_in = false; // we still want to be able to do or|equals later though
		climb_in = false;
		...
		reverse.input(reverse_in); // attempted to do a toggle
	...
	void driveLauncher() {
		...
		drive_speed = -Utility.constrain(Utility.map(Math.abs(Settings.get("idealboilerdist") - boiler_distance),0,Settings.get("maxboilerdist"),0.1,Settings.get("autodrivespd")),-0.5,0.5);
	...
	void placeGear(boolean on) { // drive backward to place the gear while aiming
		...
		if (StateMachine.isRunning("aim_gear") && 
				(Math.abs(lift_angle) < Settings.get("visiondeadzone"))) { // vision deadzone check
			StateMachine.cancel("aim_gear");
			StateMachine.start("drive_gear");
		} else if (StateMachine.isRunning("drive_gear") && 
				rear_avg.getAverage() < Settings.get("liftdist")) { // ultrasonic distance check
			StateMachine.cancel("drive_gear");
			StateMachine.start("pause_gear");
		}
		
		if (StateMachine.isRunning("aim_gear")) { // aim by turning
			aimGear();
		} else if (StateMachine.isRunning("drive_gear")) { // drive forward
			driveGear();
		} else if (StateMachine.isRunning("pause_gear")) { // pause so we don't restart the sequence
			// probably do nothing
		}
	...
	void aimGear() { // rotate the robot based on vision input
		vision_r(); // also needs LEDs on
		
		turn_speed = 0;
		
		if (lift_angle > Settings.get("visiondeadzone")) {
			turn_speed = Settings.get("turnbase") + Utility.map(Math.abs(lift_angle),0,10,0.05,Settings.get("gearaim-p")); // turn speed based on angle to the target
	...
	void driveGear() { // drive forward slowly while aiming
		vision_r(); // make sure LEDs are on so we can do some vision
	...
	void intake(boolean on) { // just run the motors to lift fuel off the ground
		if (on) {
			balllift.set(Settings.get("intakespeed")); // pretty simple to run the intake
		} else {
			balllift.set(0);
		}
	}
	...
	public void testPeriodic() {
		vision_r(); // turn on green LEDs in test so we can debug vision
		vision_f();
	}
	...
	public void robotPeriodic() {
		...
		updateArduino(); // sends bytes to arduino to tell it how to run LEDs
	}
	...
	void syncSensors() {
		...
		lift_angle = lifttracker.getNumber("angleFromGoal",0);
		boiler_distance = boilertracker.getNumber("distanceFromTarget",0); // because of update order, this may need to be put elsewhere
		...
		try { // put data into table (probably disable this during comp)
			...
			SensorPanel.report("cradle_p",cradle_prox.getValue()); // IR proximity sensor in the gear cradle
			SensorPanel.report("enc_r",launcherencoder.getRate() * 60); // launcher encoder in RPM
			SensorPanel.report("vis_bd",boiler_distance); // vision data
			SensorPanel.report("vis_ba",boiler_angle);
			SensorPanel.report("vis_ld",lift_distance);
			SensorPanel.report("vis_la",lift_angle);
			SensorPanel.report("gear_t",turn_speed); // calculated speeds for the gear routines (for debugging)
	...
	void initializeStuff() {
		...
		Settings.add("maxboilerdst", 300, 0, 1000); // mapping value
		Settings.add("maxgeardist", 100, 0, 1000); // mapping value
		...
		Settings.add("turnbase", 0.1, 0, 1); // minimum amount to turn on the motors when aiming
		...
		SensorPanel.add("vis_ld", "Lift Distance", SensorPanel.Type.BAR, 0, 400, "in");
		...
		StateMachine.add("aim_boiler","boiler"); // rotate to line up with boiler
		StateMachine.add("drive_boiler","boiler"); // drive until distance is right
		StateMachine.add("shoot_boiler","boiler"); // run the launcher
		
		StateMachine.add("indexer_on","indexer"); // unused
		...
		SmartDashboard.putData("Choose Auton Starting Position",station); // send it to dashboard
	}
}